...
AWS와 GCP는 데이터 전송량 과금 단위의 표준 정의가 다릅니다:

* **AWS (10진수 SI 기준 - Decimal):**
  * `1 GB` = 10⁹ Bytes = **1,000,000,000 바이트**
  * `1 TB` = 1,000 GB = 10¹² Bytes = **1,000,000,000,000 바이트**
* **GCP (2진수 IEC 기준 - Binary):**
  * `1 GiB` = 2³⁰ Bytes = **1,073,741,824 바이트**
  * `1 TiB` = 1,024 GiB = 2⁴⁰ Bytes = **1,099,511,627,776 바이트**

### 실제 비용 계산 시 차이점 예시
예를 들어 **100 TB (10¹⁴ 바이트)**를 GCP에서 AWS로 전송할 경우:
* GCP 청구 기준: 10¹⁴ Bytes ÷ 2³⁰ Bytes ≈ **93,132.26 GiB** × $0.042/GiB = **$3,911.55**
* 만약 100 TB를 단순히 100,000 GiB로 단순 계산하면 약 $288의 과다 청구 오차가 발생합니다.
* 본 계산기는 모든 입력값을 먼저 순수 바이트(Byte)로 정밀 변환한 후, 각 클라우드의 과금 단위에 맞춰 정확히 계산합니다.
